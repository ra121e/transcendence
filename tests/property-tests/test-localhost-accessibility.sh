...
success_rate=$(echo "scale=1; $SUCCESSFUL_REQUESTS * 100 / $TOTAL_ITERATIONS" | bc -l 2>/dev/null || echo "0")
...
for status_code in $(printf '%s\n' "${!STATUS_CODE_COUNT[@]}" | sort -n); do
    ...
    percentage=$(echo "scale=1; $count * 100 / $TOTAL_ITERATIONS" | bc -l 2>/dev/null || echo "0")
    echo "   $status_code: $count requests (${percentage}%)"
done
...
if [ "$success_rate_int" -ge 95 ]; then
    ...
    echo -e "${GREEN}📊 Achieved ${success_rate}% success rate (threshold: 95%)${NC}"
    ...
    echo -e "${RED}📊 Only achieved ${success_rate}% success rate (threshold: 95%)${NC}"
